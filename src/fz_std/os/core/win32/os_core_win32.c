...
WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) 
{
  ...
  {
    OSState.microsecond_resolution  = 1;
    LARGE_INTEGER large_int_resolution;
    if(QueryPerformanceFrequency(&large_int_resolution))
    {
      OSState.microsecond_resolution = large_int_resolution.QuadPart;
    }
  }

  {
    OS_System_Info* info = &OSState.system_info;
  ...
  {
    OS_Process_Info *info = &OSState.process_info;
